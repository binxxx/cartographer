...
class Submap : public mapping::Submap {
 public:
  Submap(int max_num_range_data,
         float high_resolution, float low_resolution,
         const transform::Rigid3d& local_pose, 
         common::ThreadPool* thread_pool,
         double initial_map_resolution,
         double distance_map_range,
         bool using_octomap = false);
  ...
  bool distance_transformed() const {return distance_transformed_;}
  std::vector<double> GetDistanceMapBoundingBox();
  ...
  void InsertRangeData(const sensor::RangeData& range_data,
                       ...
                       int high_resolution_max_range) EXCLUDES(mutex_);
  void LoadOctomap(const std::string map_file_name);
  void ComputeOctomap();
  void ComputeDistanceMap();
  void ToOctomapPointCloud(const sensor::RangeData& range_data,
                           octomap::Pointcloud& octomap_cloud);
  void Finish();

  std::shared_ptr<DynamicEDTOctomap> GetDistanceMap();
 ...
 private:
  ...
  int max_num_range_data_;
  common::Mutex mutex_;  
  std::vector<double> bounding_box_;
  std::shared_ptr<octomap::OcTree> octomap_grid_ GUARDED_BY(mutex_);
  std::shared_ptr<DynamicEDTOctomap> distmap_grid_ GUARDED_BY(mutex_); 
  // std::shared_ptr<octomap::OcTree> octomap_grid_ ;
  // std::shared_ptr<DynamicEDTOctomap> distmap_grid_ ;
  std::vector<octomap::Pointcloud> octomap_clouds_;
  std::vector<octomap::point3d> sensor_origins_;
  int high_resolution_max_range_;
  double distance_map_range_;
  bool using_octomap_;
  bool finished_ = false;
  bool distance_transformed_ = false;
};
...
class ActiveSubmaps {
 public:
  ...
  transform::Rigid3d GetMatchingSubmapLocalPose();
  std::vector<double> GetMatchingSubmapBoundingBox();
  std::vector<std::shared_ptr<Submap>> submaps() const;


 private:
  void AddSubmap(const transform::Rigid3d& local_pose) EXCLUDES(mutex_);

  const proto::SubmapsOptions options_;
  common::ThreadPool* thread_pool_;
  common::Mutex mutex_;
  int matching_submap_index_ = 0;
  std::vector<std::shared_ptr<Submap>> submaps_ GUARDED_BY(mutex_);
  // std::shared_ptr<DynamicEDTOctomap> distmap_grid_;
  std::shared_ptr<Submap> matching_submap_;
