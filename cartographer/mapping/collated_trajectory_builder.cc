...
CollatedTrajectoryBuilder::CollatedTrajectoryBuilder(
  ...
  std::cout<<"CollatedTrajectoryBuilder::CollatedTrajectoryBuilder()::Constructor" << std::endl;
...
std::shared_ptr<octomap::OcTree> CollatedTrajectoryBuilder::GetMatchingOctomap() {
  if (wrapped_trajectory_builder_!= nullptr) {
    // std::cout << "Getinng octomap\n";
    return wrapped_trajectory_builder_->GetMatchingOctomap();
  } else {
    return nullptr;
  }
}

std::shared_ptr<Submap> CollatedTrajectoryBuilder::GetMatchingSubmap() {
  if (wrapped_trajectory_builder_!= nullptr) {
      return wrapped_trajectory_builder_->GetMatchingSubmap();
  } else {
      return nullptr;
  }
}

transform::Rigid3d CollatedTrajectoryBuilder::GetMatchingLocalPose() {
  if (wrapped_trajectory_builder_!= nullptr) {
    return wrapped_trajectory_builder_->GetMatchingLocalPose();
  } else {
    LOG(WARNING) << "Failed to get wrapped_trajectory_builder_";
