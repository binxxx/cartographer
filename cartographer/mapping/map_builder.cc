...
proto::MapBuilderOptions CreateMapBuilderOptions(
  ...
  options.set_use_trajectory_builder_3d_eskf(
      parameter_dictionary->GetBool("use_trajectory_builder_3d_eskf"));
  ...
  *options.mutable_sparse_pose_graph_options() = CreateSparsePoseGraphOptions(
  ...
  CHECK_EQ(options.use_trajectory_builder_2d() +
           options.use_trajectory_builder_3d() +
           options.use_trajectory_builder_3d_eskf(), 1);
  return options;
}
...
int MapBuilder::AddTrajectoryBuilder(
  ...
  else if (options_.use_trajectory_builder_3d_eskf()) {
    CHECK(trajectory_options.has_trajectory_builder_3d_eskf_options());
    trajectory_builders_.push_back(
        common::make_unique<CollatedTrajectoryBuilder>(
            &sensor_collator_, trajectory_id, expected_sensor_ids,
            common::make_unique<mapping::GlobalTrajectoryBuilder<
                mapping_3d::ESKFLocalTrajectoryBuilder,
                mapping_3d::proto::ESKFLocalTrajectoryBuilderOptions,
                mapping_3d::SparsePoseGraph>>(
    ...
    trajectory_builders_.push_back(
        common::make_unique<CollatedTrajectoryBuilder>(
            ...
            common::make_unique<mapping::GlobalTrajectoryBuilder<
                ...
                //to change?
                sparse_pose_graph_2d_offline_.get(),
                &thread_pool_)));
  }
